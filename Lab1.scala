package jsy.student

import jsy.lab1._

object Lab1 extends jsy.util.JsyApplication with jsy.lab1.Lab1Like {
  import jsy.lab1.Parser
  import jsy.lab1.ast._

  /*
   * CSCI 3155: Lab 1
   * Julian Lambert
   *
   * Partner: Neil Nguyen
   * Collaborators: <Any Collaborators>
   */

  /*
   * Fill in the appropriate portions above by replacing things delimited
   * by '<'... '>'.
   *
   * Replace the '???' expression with your code in each function. The
   * '???' expression is a Scala expression that throws a NotImplementedError
   * exception.
   *
   * Do not make other modifications to this template, such as
   * - adding "extends App" or "extends Application" to your Lab object,
   * - adding a "main" method, and
   * - leaving any failing asserts.
   *
   * Your lab will not be graded if it does not compile.
   *
   * This template compiles without error. Before you submit comment out any
   * code that does not compile or causes a failing assert.  Simply put in a
   * '???' as needed to get something that compiles without error.
   */

  /*
   * Example: Test-driven development of plus
   *
   * A convenient, quick-and-dirty way to experiment, especially with small code
   * fragments, is to use the interactive Scala interpreter. The simplest way
   * to use the interactive Scala interpreter in IntelliJ is through a worksheet,
   * such as Lab1Worksheet.sc. A Scala Worksheet (e.g., Lab1Worksheet.sc) is code
   * evaluated in the context of the project with results for each expression
   * shown inline.
   *
   * Step 0: Sketch an implementation in Lab1.scala using ??? for unimmplemented things.
   * Step 1: Do some experimentation in Lab1Worksheet.sc.
   * Step 2: Write a test in Lab1Spec.scala, which should initially fail because of the ???.
   * Step 3: Fill in the ??? to finish the implementation to make your test pass.
   */

  //def plus(x: Int, y: Int): Int = ???
  def plus(x: Int, y: Int): Int = x + y //Just add

  /* Exercises */

  def abs(n: Double): Double = {
    if (n < 0) -n else n //if negative, then use opposite
  }

  def xor(a: Boolean, b: Boolean): Boolean = {
    if (a == b) false else true // xor is false when 11 or 00, true when 10 or 01
  }

  def repeat(s: String, n: Int): String =  {
    require(n >= 0) //requires >0 repetitions
    if (n == 0) "" else s + repeat(s, n - 1) //repeats string while counting down repetitons
  }

  def sqrtStep(c: Double, xn: Double): Double = xn - (xn*xn-c)/(2*xn) //performs squre root step usign newton's method

  def sqrtN(c: Double, x0: Double, n: Int): Double =  {
    require(n >= 0) //avoid mathematical errors
    if (n == 0) x0 else { //checks for n value
      sqrtN(c, sqrtStep(c, x0), n - 1) //does sqrtStep n times
    }
  }

  def sqrtErr(c: Double, x0: Double, epsilon: Double): Double =  {
    require(epsilon > 0) //require an epsilon of greater than 0 value
    if ((x0 * x0 - c) * (x0 * x0 - c) < epsilon * epsilon) x0 else { //finds sqrt with error less than epsilon
      sqrtErr(c, sqrtStep(c, x0), epsilon) //recursive call
    }
  }


  def sqrt(c: Double): Double = {
    require(c >= 0)
    if (c == 0) 0 else sqrtErr(c, 1.0, 0.0001) //finds sqrt with small error
  }

  /* Search Tree */

  // Defined in Lab1Like.scala:
  //
  // sealed abstract class SearchTree
  // case object Empty extends SearchTree
  // case class Node(l: SearchTree, d: Int, r: SearchTree) extends SearchTree

  def repOk(t: SearchTree): Boolean = {
    def check(t: SearchTree, min: Int, max: Int): Boolean = t match { //checks tree
      case Empty => true  //return true if empty.
      case Node(l, d, r) => {
        if((min <= d) && (max > d)) //checks value of d withing constraints
          return check(l, min, d) && check(r, d, max) //checks left min with d and right tree max with d
        else //no valid search tree
          return false
      }
    }
    check(t, Int.MinValue, Int.MaxValue) //checks tree bounds
  }

  def insert(t: SearchTree, n: Int): SearchTree = {
    (t: @unchecked) match {
      case Empty => Node(Empty, n, Empty) //case in which tree is empty, new tree
      case Node(l, d, r) => {
        if(n >= d) //checks value of inputed node and current
          return Node(l, d, insert(r, n)) //values greater than or equal to current nodes go on right
        else
          return Node(insert(l, n), d, r) //values less than or equal to current nodes go left
      }
    }
  }

  def deleteMin(t: SearchTree): (SearchTree, Int) = {
    require(t != Empty) //tree cannot be empty
    (t: @unchecked) match {
      case Node(Empty, d, r) => (r, d) //no left children, this must be min. return right tree
      case Node(l, d, r) => //has left children
        val (l1, m) = deleteMin(l) //recurse down left child of tree
        (Node(l1, d, r), m) //return node with new left branch and min value
    }
  }

  def delete(t: SearchTree, n: Int): SearchTree = t match {
    case Empty => Empty //if no elements remain, return empty
    case Node(l, d, r) => if (n < d) { //target less than current node
      Node(delete(l,n),d,r) //recurse down left path
    } else if (n == d) { //target found
      if (r != Empty) { //if r has right children...
        val (r1, m) = deleteMin(r) //...run deleteMin on right tree
        Node(l, m, r1) //return Node with modified right tree and swapped min value
      } else { //no right children
        if (l != Empty) l else Empty //either return left tree (left children) or nothing (no children)
      }
    } else { //target greater than current node
      Node(l, d, delete(r,n)) //recurse down right path
    }
  }

  /* JavaScripty */

  def eval(e: Expr): Double = e match {
    case N(n) => n //parenthesis
    case Unary(Neg, e1) => { //get opposite of e1
      if (eval(e1) > 0) //if greater than 0
        -eval(e1) //negative
      else
        eval(e1) //if negative, then positive
    } //PEMDAS
    case Binary(Plus, e1, e2) => eval(e1) + eval(e2) //addition
    case Binary(Minus, e1, e2) => eval(e1) - eval(e2) //subtraction
    case Binary(Times, e1, e2) => eval(e1) * eval(e2) //multiplication
    case Binary(Div, e1, e2) => eval(e1) / eval(e2) //division
    
  }

 // Interface to run your interpreter from a string.  This is convenient
 // for unit testing.
 def eval(s: String): Double = eval(Parser.parse(s))



 /* Interface to run your interpreter from the command-line.  You can ignore the code below. */

 def processFile(file: java.io.File) {
    if (debug) { println("Parsing ...") }

    val expr = Parser.parseFile(file)

    if (debug) {
      println("\nExpression AST:\n  " + expr)
      println("------------------------------------------------------------")
    }

    if (debug) { println("Evaluating ...") }

    val v = eval(expr)

    println(prettyNumber(v))
  }

}
